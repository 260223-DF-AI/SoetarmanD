...
def run_metrics_lab():
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            # TODO: Calculate TP (true positives), FP (false positives), and FN (false negatives) using basic tensor math
            # Hint for TP: count where binary_preds == 1 AND y_val == 1
            TP = torch.sum((binary_preds == 1) & (y_val == 1))
            FP = torch.sum((binary_preds == 1) & (y_val == 0))
            FN = torch.sum((binary_preds == 0) & (y_val == 1))
